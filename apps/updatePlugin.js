...
export class Update extends plugin {
    ...
    async runUpdate(isForce) {
        const _path = './plugins/bandori-plugin/'
        ...
        this.oldCommitId = await this.getcommitId('bandori-plugin')
        uping = true
        let ret = await this.execSync(command)
        uping = false

        if (ret.error) {
            logger.mark(`${this.e.logFnc} 更新失败：邦邦插件`)
            this.gitErr(ret.error, ret.stdout)
            return false
        }
        let time = await this.getTime('bandori-plugin')

        if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
            await this.reply(`邦邦插件已经是最新版本\n最后更新时间：${time}`)
        } else {
            await this.reply(`邦邦插件\n最后更新时间：${time}`)
            this.isUp = true
            let log = await this.getLog('bandori-plugin')
