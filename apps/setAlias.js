...
export class abbrSet extends plugin {
  constructor() {
    ...
    this.file = './plugins/bandori-plugin/config/alias.yaml'
  }
  async addAlias() {
    if (!await this.checkAuth()) return
    let keyName = this.e.msg.replace(new RegExp(`#|邦邦|邦多利|设置|配置|添加|别名|昵称(.*)`, 'g'), '').trim()
  ...
  async delAlias() {
    if (!await this.checkAuth()) return
    let inputName = this.e.msg.replace(new RegExp(`#|邦邦|邦多利|删除|别名|昵称`, 'g'), '').trim()
  ...
  async aliasList() {
    ...
    let keyName = this.e.msg.replace(new RegExp(`#|邦邦|邦多利|别名|昵称`, 'g'), '').trim()
    let result = {}
    Object.entries(role).some(([name, aliases]) => {
      if (aliases.includes(keyName) || name == keyName) {
        result = { name: aliases[0], aliases: aliases }
        return true
      }
    })
